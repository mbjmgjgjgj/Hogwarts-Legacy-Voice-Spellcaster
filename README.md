...
Этот проект позволяет управлять заклинаниями в игре **Hogwarts Legacy** при помощи голоса. Скажи `Люмос` — и палочка зажжётся. `Экспелиармус` — и враг будет обезоружен.  
...
## 💡 Функционал
* 🎙️ Голосовое распознавание `Vosk` (Offline)
* 🧩 Модель-классификатор для распознавания заклинаний
* 🔊 Звуковое сопровождение для заклинаний (Аудио `TTS Silero` - `ru_v3`)
...
`MIN_CONFIDENCE` - Параметр минимального уровеня точности для применения заклинания. Чем выше параметр, тем меньше ложных срабатываний, однако, тем и меньше положительных. 
